...
def signup():
    try:
        ...
        if not application_id or not client_id:
            return jsonify({'message': 'Missing headers'}), 400
...
def verify_email(token):
    ...
    if user:
        email, fullname, password, username, phone_number, application_id, client_id = user

        create_user(fullname, username, password, email, phone_number, application_id, client_id)
        delete_entry(email)
...
def change_password(current_user):
    ...
    if stored_hashed_password and check_password_hash(stored_hashed_password, old_password):
        ...
        change_user_password(current_user, hashed_new_password)
